"""
Live sanity check for schema-addressable mode (SIGMOD guide Sec. 10).
...
def edgar_sanity() -> None:
    ...
    for cik in keys:
        ...
        for s in srcs:
            ...
            print(f"     {s.url}")
            # Sec. 10 stamps: these four fields are what hand corroboration
            # its supersession edges and lift the q-bar cap.
            print(f"     doc_type={s.doc_type!r}  chain={s.authority_chain_id!r}")
            print(f"     class={s.source_class}  anchored={s.identity_anchored}")
        if srcs:
